# FZ1_PVR_006 — Chaikin money-flow proxy 20

- Family: `PRICE_VOLUME_RELATION`
- Concept: `MONEY_FLOW_LOCATION_VOLUME`
- Parameterization group: `FZ1_PVR_006` / `SINGLE`
...
`sum(((2*C-H-L)/(H-L))*volume,20)/(sum(volume,20)+1e-12)`
...
Closing location weighted by volume may distinguish accumulation from distribution inside daily ranges.
